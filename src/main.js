...
/* ---------- 그리기 ---------- */
function draw(){
  if(phase==='cover'){drawCover();return;}
  ctxL.clearRect(0,0,400,600); ctxR.clearRect(0,0,400,600);
  ctxL.drawImage(SPR.background,0,0,400,600); ctxR.drawImage(SPR.background,0,0,400,600);
  const d=(ctx,img,x,y)=>ctx.drawImage(img,x,y,32,32);
  state.items.forEach(i=>{d(ctxL,SPR[i.type],i.x,i.y);d(ctxR,SPR[i.type],i.x,i.y);});
  state.obstacles.forEach(o=>{d(ctxL,SPR[o.type],o.x,o.y);d(ctxR,SPR[o.type],o.x,o.y);});
  const r=state.players.rabbit,t=state.players.turtle;
  d(ctxL,SPR.rabbit,r.x,r.y); d(ctxR,SPR.turtle,t.x,t.y);
  ctxL.fillStyle='black';ctxL.font='16px Arial';
  ctxL.fillText(`체력:${r.stamina} 줄:${r.row.toFixed(1)}`,10,20);
  ctxR.fillText(`체력:${t.stamina} 줄:${t.row.toFixed(1)}`,10,20);
  if(state.isWin){ctxL.fillStyle='orange';ctxL.font='24px Arial';ctxL.fillText('축하해요! 클리어!',60,300);stopTimer();recordCheck();}
  if(state.isOver){ctxL.fillStyle='red';ctxL.font='30px Arial';ctxL.fillText('Game Over',80,300);stopTimer();}
}
